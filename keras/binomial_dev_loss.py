...
def binomial_dev(y_true,y_pred,batch_size = 128):
    x,y = y_pred
    ...
    vcost = K.sum(K.log(-1*alpha*(K.exp((vconn - beta)*vmat)) + 1)*vmat2,axis = [0,1])                    
    return([vcost])
 

def loss_diff(vects,batch_size):
    x,y = vects
...
def cosine_distance(vects):
    ...
    vcost = loss_diff(vects,batch_size = 128)
    return ([K.mean(vunit1*vunit2, axis = -1),vcost])
...
# train
rms = RMSprop()
model.compile(loss=binomial_dev,optimizer=rms)
